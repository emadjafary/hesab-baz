...
import {
  LayoutDashboard, Wallet, PieChart, ArrowLeftRight, BarChart3,
  Users, TrendingUp, Tag, Building2, Calculator, CalendarDays,
  Settings, Calendar, LogOut, type LucideIcon,
} from 'lucide-react'
    ...
    <aside className="hidden md:flex md:w-56 flex-col bg-white border-s border-slate-200 h-screen sticky top-0">
            ...
            return (
              <li key={item.href}>
                ...
                >
                  ...
                  {active && <span className="ms-auto h-1.5 w-1.5 rounded-full bg-brand-500" />}
                </Link>
              </li>
            )
          })}
      ...
      <div className="p-3 border-t border-slate-100 space-y-1">
        <button
          ...
          className="w-full flex items-center gap-3 rounded-lg px-3 py-2 text-sm text-slate-500 hover:bg-rose-50 hover:text-rose-600 transition-colors"
        >
          <LogOut size={16} strokeWidth={1.5} />
          <span>خروج از حساب</span>
        </button>
        <p className="text-xs text-slate-400 px-3">نسخه ۱۰.۰</p>
      </div>
    </aside>
  )
}
