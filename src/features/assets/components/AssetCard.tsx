...
import { Trash2, Pencil, RefreshCw, ExternalLink } from 'lucide-react'
...
export const AssetCard = React.memo(function AssetCard({
  ...
  return (
    ...
    >
      <div className="p-4">
        {/* Header: actions (left) + title+icon (right, in RTL) */}
        ...
        {/* Body: مقدار / قیمت هر واحد / ارزش کل */}
        ...
        <div className="mt-3 text-[11px] leading-relaxed text-slate-400 space-y-0.5">
          ...
          <div>
            زمان قیمت در:{' '}
            <span className="text-slate-500">{formatClockFa(asset.lastUpdate)}</span>
          </div>
          <div>
            آخرین دریافت در سامانه:{' '}
            <span className="text-slate-500">{timeAgoFa(asset.lastUpdate)}</span>
          </div>
        </div>
      </div>

      {/* Refresh button — full width chip at the bottom */}
      <button
  ...
  return (
    <div className="flex items-center justify-between">
      ...
      <dd
        className={cn(
          'tabular-nums',
          subtle ? 'text-sm text-slate-600' : 'text-sm font-semibold text-slate-800',
        )}
      >
        {value}
      </dd>
